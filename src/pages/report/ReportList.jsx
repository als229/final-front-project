...
const ReportList = () => {
  ...
  const fetchReportList = useCallback(() => {
    axios
      ...
      .then((res) => {
        ...
        else { alert(res.data ? `${res.data.code} ${res.data.message}` : '신고 목록 조회에 실패했습니다.'); }
      })
      .catch((err) => {
  ...
  useEffect(() => {
    if(auth.accessToken) { fetchReportList(); }
            ...
            <tbody>
              {reportList.map((list) => (
                <TableRow key={list.reportNo}>
                  ...
                  <TableData>
                    <TableButton onClick={() => handleOpenModal(list)}>열람</TableButton>
                  </TableData>
                </TableRow>
              ))}
            </tbody>
