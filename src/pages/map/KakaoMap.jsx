...
const KakaoMap = ({contentId}) => {
  ...
  const kakaoMapApiKey = window.ENV?.KAKAO_MAP_API_KEY;
    ...
    axios
      ...
      .then((res) => {
        if(res.data && res.data.items) {
          ...
          if( isNaN(lat) || isNaN(lng)) { /* 좌표가 유효한 숫자가 아닌 경우 */
            console.warn(`contentId ${id}에 대한 유효한 좌표(mapX, mapY)가 없어 임시 좌표를 사용합니다.`);
            lat = DEFAULT_LAT;
            lng = DEFAULT_LNG;
            setError("해당 콘텐츠의 정확한 위치 정보가 없어 기본 위치를 표시합니다.");
          }
          else { setError(null); }
          setMapData({
          ...
          console.warn(`contentId ${id}에 대한 지도 데이터를 가져오지 못했거나 응답이 비정상적입니다.`);
          setError(res.data?.message || "지도 데이터를 가져오지 못했습니다. 기본 위치를 표시합니다.");
          setMapData({
            center: { lat: DEFAULT_LAT, lng: DEFAULT_LNG },
            markers: [{ lat: DEFAULT_LAT, lng: DEFAULT_LNG, title: DEFAULT_TITLE }],
          });
        }
      })
      .catch((err) => {
        console.error("지도 데이터 조회 중 오류 발생:", err);
        setError("지도 데이터를 불러오는 중 오류가 발생했습니다. 기본 위치를 표시합니다.");
        setMapData({
          center: { lat: DEFAULT_LAT, lng: DEFAULT_LNG },
          markers: [{ lat: DEFAULT_LAT, lng: DEFAULT_LNG, title: DEFAULT_TITLE }],
        });
  ...
  useEffect(() => {
    ...
    if(loading || !mapData.center) { return; }
    ...
    const loadKakaoMap = () => {
      window.kakao.maps.load(() => {
        ...
        const center = new window.kakao.maps.LatLng(mapData.center.lat, mapData.center.lng);
        const options = {
          center: center,
          level: 3, /* 지도의 확대 레벨 */
        };
        if( !kakaoMap.current) {
          ...
          window.kakao.maps.event.addListener(kakaoMap.current, 'center_changed', function() {
            const currentCenter = kakaoMap.current.getCenter(); 
            console.log(`지도의 중심 좌표가 변경되었습니다: 위도 ${currentCenter.getLat()}, 경도 ${currentCenter.getLng()}`);
          });
          window.kakao.maps.event.addListener(kakaoMap.current, 'zoom_changed', function() {
        ...
        mapData.markers.forEach(markerInfo => {
          const markerPosition = new window.kakao.maps.LatLng(markerInfo.lat, markerInfo.lng);
    ...
    if( !window.kakao || !window.kakao.maps) {
      ...
      return () => {
        ...
        if(kakaoMap.current) { kakaoMap.current = null; }
      };
    } 
    else {
  ...
  if( !mapData.center && error) {
    return (
      <MapWrapper> 
        <MessageContainer $isError>지도 정보를 표시할 수 없습니다.</MessageContainer>;
      </MapWrapper>
    );
  }
