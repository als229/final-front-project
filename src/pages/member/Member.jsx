...
const Member = ({
  member, isOpen, onClose, onUpdateSuccess 
}) => {
  ...
  const handleNicknameChange = () => {
    ...
    axios
      ...
      .then((res) => {
        ...
        else { alert(res.data ? `${res.data.code} ${res.data.message}` : '닉네임 변경에 실패했습니다.'); }
      })
      .catch((err) => {
  ...
  const handleStatusChange = () => {
    ...
    .then((response) => {
      if (response.status === 200 && response.data && response.data.code === "M101") {
        alert('회원 상태가 성공적으로 변경되었습니다.');
        onUpdateSuccess();
      } 
      else { alert(response.data ? `${response.data.code} ${response.data.message}` : '회원 상태 변경에 실패했습니다.'); }
    })
    .catch((err) => {
  ...
  return (
    <Modal 
      ...
      <MemberInfo>
        <p><strong>회원 번호:</strong> {member.userNo}</p>
        <p><strong>아이디:</strong> {member.userId}</p>
        <p><strong>현재 닉네임:</strong> {member.nickname}</p>
        <p><strong>현재 상태:</strong> {member.status === 'Y' ? '활성' : '비활성'}</p>
      </MemberInfo>
